...
- **SNI Domain Routing** — Each tunnel gets a unique subdomain (e.g., `agent-1.tunneledge.dev`), routed via TLS SNI on a single port
...
2. Client connects to `agent-1.tunneledge.dev:443` via TLS
...
2. Gateway registers tunnel, assigns subdomain `agent-1.tunneledge.dev`
3. Public client connects via TLS to `agent-1.tunneledge.dev:443`
...
1. Public client connects to `agent-1.tunneledge.dev:443` via TLS
...
echo "127.0.0.1 agent-1.tunneledge.dev" | sudo tee -a /etc/hosts

# Connect via TLS SNI
echo "Hello TunnelEdge" | openssl s_client -connect agent-1.tunneledge.dev:443 \
  -servername agent-1.tunneledge.dev -quiet
```
